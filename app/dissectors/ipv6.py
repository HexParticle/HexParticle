...
class IPV6DissectorComponent:
    ...
    def dissect(parent_node, ip_header, previous_node = None):
        if isinstance(ip_header, protos.IPV6ExtHeader):
            IPV6ExtDissectorComponent.dissect(previous_node, ip_header)
            return
        ...
        widgets.QTreeWidgetItem(ip_item, ["Next Header", next_hdr if next_hdr else f"{ip_header.next_hdr} (Unknown next header type)"])
...
class IPV6ExtDissectorComponent:
    @staticmethod
    def dissect(parent_node, ext_header):

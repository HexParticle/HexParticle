...
class TCPHeader(ctypes.Structure):
    ...
    def nop(self):
        for option in self.options:
            if option.kind == tcp.TCP_OPTION_NOP:
                return True
        return False

    def mss(self):
        for option in self.options:
            if option.kind == tcp.TCP_OPTION_MSS:
                return option.data
        return None

    def window_scale(self):
        for option in self.options:
            if int(option.kind) == tcp.TCP_OPTION_WINDOW_SCALE:
                return option.data
        return None

    def sack_premitted(self):
        for option in self.options:
            if int(option.kind) == tcp.TCP_OPTION_SACK_PERMITTED:
                return option.data
        return None

    def sack(self):
        for option in self.options:
            if option.kind == tcp.TCP_OPTION_SACK:
                return option.data
        return None

    def timestamps(self):
        for option in self.options:
            if option.kind == tcp.TCP_OPTION_TIMESTAMPS:
                timestamp = 0
